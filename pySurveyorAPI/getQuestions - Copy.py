...
# params: sessionId, qgroup
    ...
    for row in rows:
        ...
        query3 = '''SELECT sm.Id SurveyMasterId, q.Id QuestionId, q.QuestionText, ag.Id AnswerGroupId, 
        ...
        for row3 in rows3:
            opt={}           
            opt['id'] = row3.AnswerOptionId
            opt['text'] = row3.AnswerText

            # get counts
            query2 = 'SELECT COUNT(*) Tally FROM Result WHERE SurveyMasterId = ' + str(survey)
            query2 += ' AND QuestionId = ' + str(row.QuestionId) 
            query2 += ' AND AnswerOptionId = ' + str(row3.AnswerOptionId)
